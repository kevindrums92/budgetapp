...
test.describe('Release Features - v0.7.0', () => {
  ...
  test('CategoryMonthDetailPage - should navigate from Stats to category detail', async ({ page }) => {
    ...
    // Click on a category in the legend (should be Comida)
    await page.click('button:has-text("Mercado")').first();
  ...
  test('BudgetOnboardingWizard - should show on first visit and allow drag', async ({ page }) => {
    // Clear the onboarding seen flag
    await page.evaluate(() => {
    ...
    // Wait a bit for wizard to appear (has animation)
    await page.waitForTimeout(500);

    // Wizard should be visible
    await expect(page.locator('text=Bienvenido al Budget')).toBeVisible();
    ...
    await expect(page.locator('text=Establece Límites')).toBeVisible();

    // Click "Siguiente" again
    await page.click('button:has-text("Siguiente")');

    // Third slide
    await expect(page.locator('text=Monitorea tu Progreso')).toBeVisible();

    // Click "Siguiente" again
    await page.click('button:has-text("Siguiente")');

    // Fourth (last) slide
    await expect(page.locator('text=Balance vs Budget')).toBeVisible();
